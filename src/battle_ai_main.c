...
static u32 Ai_SetMoveAccuracy(struct AiLogicData *aiData, u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_NO_GUARD, aiData) || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_NO_GUARD, aiData) || gMovesInfo[move].accuracy == 0) // Moves with accuracy 0 or no guard ability always hit.
        accuracy = 100;
    else
...
static bool32 AI_ShouldSwitchIfBadMoves(u32 battler, bool32 doubleBattle)
{
        ...
        && !(AI_THINKING_STRUCT->aiFlags[battler] & AI_FLAG_SEQUENCE_SWITCHING))
    {
        ...
        if (AI_BattlerHasAbilityOrInnate(battler, ABILITY_TRUANT, AI_DATA)
...
static s32 AI_CheckBadMove(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (!(moveTarget & MOVE_TARGET_USER))
    {
        ...
        {
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_MAGIC_GUARD, aiData)) {
                switch (moveEffect)
                {
                case EFFECT_POISON:
            ...
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_MAGIC_BOUNCE, aiData)
             && gMovesInfo[move].magicCoatAffected) {
                RETURN_SCORE_MINUS(20);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_CONTRARY, aiData)
             && IsStatLoweringEffect(moveEffect)) {
                RETURN_SCORE_MINUS(20);
            }
            if (((AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_LIGHTNING_ROD, aiData) && B_REDIRECT_ABILITY_IMMUNITY > GEN_4)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_MOTOR_DRIVE, aiData)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_VOLT_ABSORB, aiData))
             && moveType == TYPE_ELECTRIC) {
                RETURN_SCORE_MINUS(20);
            }
            if (((AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_STORM_DRAIN, aiData) && B_REDIRECT_ABILITY_IMMUNITY > GEN_4)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_WATER_ABSORB, aiData)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_DRY_SKIN, aiData))
             && moveType == TYPE_WATER) {
                RETURN_SCORE_MINUS(20);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_FLASH_FIRE, aiData)
             && moveType == TYPE_FIRE) {
                RETURN_SCORE_MINUS(20);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_EARTH_EATER, aiData)
             && moveType == TYPE_GROUND) {
                RETURN_SCORE_MINUS(20);
            }
                        if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_COMATOSE, aiData)
             && IsNonVolatileStatusMoveEffect(moveEffect)) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_SHIELDS_DOWN, aiData)
             && IsShieldsDownProtected(battlerAtk) && IsNonVolatileStatusMoveEffect(moveEffect)) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_LEAF_GUARD, aiData)
             && ((AI_GetWeather(aiData) & B_WEATHER_SUN)
             && aiData->holdEffects[battlerDef] != HOLD_EFFECT_UTILITY_UMBRELLA
             && IsNonVolatileStatusMoveEffect(moveEffect))) {
                RETURN_SCORE_MINUS(10);
            }
            if ((AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_CLEAR_BODY, aiData)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_FULL_METAL_BODY, aiData)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_WHITE_SMOKE, aiData))
             && IsStatLoweringEffect(moveEffect)) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_HYPER_CUTTER, aiData)
            ...
            if (((AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_ILLUMINATE, aiData) && B_ILLUMINATE_EFFECT > GEN_8)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_KEEN_EYE, aiData)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_MINDS_EYE, aiData))
             && (moveEffect == EFFECT_ACCURACY_DOWN || moveEffect == EFFECT_ACCURACY_DOWN_2)) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_BIG_PECKS, aiData)
             && (moveEffect == EFFECT_DEFENSE_DOWN || moveEffect == EFFECT_DEFENSE_DOWN_2)) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_JUSTIFIED, aiData)
             && moveType == TYPE_DARK && !IS_MOVE_STATUS(move)) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_RATTLED, aiData)
             && !IS_MOVE_STATUS(move)
             && (moveType == TYPE_DARK || moveType == TYPE_GHOST || moveType == TYPE_BUG)) {
                RETURN_SCORE_MINUS(10);
            }
            if ((AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_DAZZLING, aiData)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_QUEENLY_MAJESTY, aiData)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_ARMOR_TAIL, aiData))
             && atkPriority > 0) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_AROMA_VEIL, aiData)
             && IsAromaVeilProtectedMove(move)) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_SWEET_VEIL, aiData)
             && (moveEffect == EFFECT_SLEEP || moveEffect == EFFECT_YAWN)) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_FLOWER_VEIL, aiData)
             && IS_BATTLER_OF_TYPE(battlerDef, TYPE_GRASS)
             && (IsNonVolatileStatusMoveEffect(moveEffect) || IsStatLoweringEffect(moveEffect))) {
                RETURN_SCORE_MINUS(10);
            }
            if ((AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_DEFIANT, aiData)
             || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_COMPETITIVE, aiData))
            ...
            if (isDoubleBattle)
            {
                if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerDef), ABILITY_LIGHTNING_ROD, aiData)
                 && moveType == TYPE_ELECTRIC && !IsMoveRedirectionPrevented(move, aiData->abilities[battlerAtk])) {
                    RETURN_SCORE_MINUS(20);
                }
                if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerDef), ABILITY_STORM_DRAIN, aiData)
                 && moveType == TYPE_WATER && !IsMoveRedirectionPrevented(move, aiData->abilities[battlerAtk])) {
                    RETURN_SCORE_MINUS(20);
                }
                if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerDef), ABILITY_MAGIC_BOUNCE, aiData)
                 && gMovesInfo[move].magicCoatAffected && moveTarget & (MOVE_TARGET_BOTH | MOVE_TARGET_FOES_AND_ALLY | MOVE_TARGET_OPPONENTS_FIELD)) {
                    RETURN_SCORE_MINUS(20);
                }
                if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerDef), ABILITY_SWEET_VEIL, aiData)
                 && (moveEffect == EFFECT_SLEEP || moveEffect == EFFECT_YAWN)) {
                    RETURN_SCORE_MINUS(20);
                }
                if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerDef), ABILITY_FLOWER_VEIL, aiData)
                 && (IS_BATTLER_OF_TYPE(battlerDef, TYPE_GRASS)) && (IsNonVolatileStatusMoveEffect(moveEffect) || IsStatLoweringEffect(moveEffect))) {
                    RETURN_SCORE_MINUS(10);
                }
                if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerDef), ABILITY_AROMA_VEIL, aiData)
                 && IsAromaVeilProtectedMove(move)) {
                    RETURN_SCORE_MINUS(10);
                }
                if ((AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerDef), ABILITY_DAZZLING, aiData)
                 || AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerDef), ABILITY_QUEENLY_MAJESTY, aiData)
                 || AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerDef), ABILITY_ARMOR_TAIL, aiData))
                 && atkPriority > 0) {
                    RETURN_SCORE_MINUS(10);
                }
        ...
        if (B_PRANKSTER_DARK_TYPES >= GEN_7 && IS_BATTLER_OF_TYPE(battlerDef, TYPE_DARK)
          && AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_PRANKSTER, aiData) && IS_MOVE_STATUS(move)
...
// the following checks apply to any target (including user)
    ...
    // stat raising effects
        ...
        case EFFECT_ATTACK_ACCURACY_UP: //hone claws
            if (!AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_CONTRARY, aiData))
            {
        ...
        case EFFECT_ROTOTILLER:
            if (isDoubleBattle)
            {
                if (!(IS_BATTLER_OF_TYPE(battlerAtk, TYPE_GRASS)
                  ...
                  && !AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_CONTRARY, aiData)
            ...
            break;
        case EFFECT_GEAR_UP:
            if (AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_PLUS, aiData) || AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_MINUS, aiData))
            {
            ...
            if (isDoubleBattle)
            {
                ...
                else if (!AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_PLUS, aiData) && !AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_MINUS, aiData))
                {
        ...
        case EFFECT_MAGNETIC_FLUX:
            ...
            if (isDoubleBattle)
            {
                if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_PLUS, aiData) || AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_MINUS, aiData))
                {
                ...
                else if (!AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_PLUS, aiData) && !AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_MINUS, aiData))
                {
                    ADJUST_SCORE(-10);    // nor our or our partner's ability is plus/minus
                }
            }
    ...
    // stat lowering effects
        ...
        case EFFECT_ATTACK_DOWN_2:
            if (!ShouldLowerStat(battlerDef, aiData->abilities[battlerDef], STAT_ATK)) //|| !HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL))
                ADJUST_SCORE(-10);
            else if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_HYPER_CUTTER, aiData))
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_SPEED_DOWN_2:
            if (!ShouldLowerStat(battlerDef, aiData->abilities[battlerDef], STAT_SPEED))
                ADJUST_SCORE(-10);
            else if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_SPEED_BOOST, aiData))
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_ACCURACY_DOWN_2:
            if (!ShouldLowerStat(battlerDef, aiData->abilities[battlerDef], STAT_ACC))
                ADJUST_SCORE(-10);
            else if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_KEEN_EYE, aiData) || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_MINDS_EYE, aiData)
                        || (B_ILLUMINATE_EFFECT >= GEN_9 && AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_ILLUMINATE, aiData)))
                ADJUST_SCORE(-8);
            break;
    ...
    // other
        ...
        case EFFECT_FOCUS_PUNCH:
            ...
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_WONDER_GUARD, aiData) && effectiveness < AI_EFFECTIVENESS_x2)
                ADJUST_SCORE(-10);
            break;
        case EFFECT_COUNTER:
        case EFFECT_MIRROR_COAT:
        ...
        case EFFECT_ROAR:
            if (CountUsablePartyMons(battlerDef) == 0)
                ADJUST_SCORE(-10);
            else if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_SUCTION_CUPS, aiData))
                ADJUST_SCORE(-10);
        ...
        case EFFECT_SUBSTITUTE:
            if (gBattleMons[battlerAtk].status2 & STATUS2_SUBSTITUTE || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_INFILTRATOR, aiData))
                ADJUST_SCORE(-8);
        ...
        case EFFECT_LEECH_SEED:
            if (gStatuses3[battlerDef] & STATUS3_LEECHSEED
             ...
             || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_SCORE(-10);
            else if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_LIQUID_OOZE, aiData))
                ADJUST_SCORE(-3);
            break;
        case EFFECT_DISABLE:
        ...
        case EFFECT_PERISH_SONG:
            if (isDoubleBattle)
            {
                if (CountUsablePartyMons(battlerAtk) == 0
                  && !AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_SOUNDPROOF, aiData)
                  && !AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_SOUNDPROOF, aiData)
                  && CountUsablePartyMons(FOE(battlerAtk)) >= 1)
                {
                    ADJUST_SCORE(-10); //Don't wipe your team if you're going to lose
                }
                else if ((!IsBattlerAlive(FOE(battlerAtk)) || AI_BattlerHasAbilityOrInnate(FOE(battlerAtk), ABILITY_SOUNDPROOF, aiData)
                  || gStatuses3[FOE(battlerAtk)] & STATUS3_PERISH_SONG)
                  && (!IsBattlerAlive(BATTLE_PARTNER(FOE(battlerAtk))) || AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(FOE(battlerAtk)), ABILITY_SOUNDPROOF, aiData)
            ...
            {
                if (CountUsablePartyMons(battlerAtk) == 0 && !AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_SOUNDPROOF, aiData)
                  && CountUsablePartyMons(battlerDef) >= 1)
                    ADJUST_SCORE(-10);

                if (gStatuses3[FOE(battlerAtk)] & STATUS3_PERISH_SONG || AI_BattlerHasAbilityOrInnate(FOE(battlerAtk), ABILITY_SOUNDPROOF, aiData))
                    ADJUST_SCORE(-10);
            }
            break;
        ...
        case EFFECT_MAGNITUDE:
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_LEVITATE, aiData))
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_KNOCK_OFF:
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_STICKY_HOLD, aiData))
                ADJUST_SCORE(-10);
            break;
        case EFFECT_INGRAIN:
        ...
        case EFFECT_WATER_SPORT:
            ...
            break;
        case EFFECT_ABSORB:
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_LIQUID_OOZE, aiData))
                ADJUST_SCORE(-6);
            break;
        ...
        case EFFECT_LOCK_ON:
            if (gStatuses3[battlerDef] & STATUS3_ALWAYS_HITS
              || AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_NO_GUARD, aiData)
              || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_NO_GUARD, aiData)
              || DoesPartnerHaveSameMoveEffect(BATTLE_PARTNER(battlerAtk), battlerDef, move, aiData->partnerMove))
                ADJUST_SCORE(-10);
            break;
        case EFFECT_LASER_FOCUS:
            if (gStatuses3[battlerAtk] & STATUS3_LASER_FOCUS)
                ADJUST_SCORE(-10);
            else if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_SHELL_ARMOR, aiData) || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_BATTLE_ARMOR, aiData))
                ADJUST_SCORE(-8);
            break;
        case EFFECT_SKETCH:
            if (gLastMoves[battlerDef] == MOVE_NONE)
        ...
        case EFFECT_PROTECT:
            {
                  ...
                  && move != MOVE_CRAFTY_SHIELD) //These moves have infinite usage
                {
                    if (GetBattlerSecondaryDamage(battlerAtk) >= gBattleMons[battlerAtk].hp
                      && !AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_MOXIE, aiData)
                      && !AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_BEAST_BOOST, aiData))
                    {
        ...
        case EFFECT_DEFOG:
            ...
            if (gBattleMons[battlerDef].statStages[STAT_EVASION] == MIN_STAT_STAGE
              || ((AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_CONTRARY, aiData)) && !IS_TARGETING_PARTNER(battlerAtk, battlerDef))) // don't want to raise target stats unless its your partner
                ADJUST_SCORE(-10);
            break;
        ...
        case EFFECT_WORRY_SEED:
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_INSOMNIA, aiData)
        ...
        case EFFECT_NATURAL_GIFT:
            if (AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_KLUTZ, aiData)
        ...
        case EFFECT_FLING:
            ...
            break;
        case EFFECT_EMBARGO:
            if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_KLUTZ, aiData)
              || gFieldStatuses & STATUS_FIELD_MAGIC_ROOM
        ...
        case EFFECT_SPICY_EXTRACT:
            if (battlerAtk != BATTLE_PARTNER(battlerDef)
             && (HasMoveWithCategory(battlerDef, DAMAGE_CATEGORY_PHYSICAL)
              || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_CLEAR_BODY, aiData)
              || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_GOOD_AS_GOLD, aiData)
    ...
    // Choice items
    if (HOLD_EFFECT_CHOICE(aiData->holdEffects[battlerAtk]) && gBattleMons[battlerAtk].ability != ABILITY_KLUTZ && !SpeciesHasInnate(gBattleMons[battlerAtk].species, ABILITY_KLUTZ))
    {
...
static s32 AI_DoubleBattle(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (gMovesInfo[aiData->partnerMove].alwaysCriticalHit && AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_ANGER_POINT, aiData))
    {
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        ...
        {
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_VOLT_ABSORB, aiData)
             && !(AI_THINKING_STRUCT->aiFlags[battlerAtk] & AI_FLAG_HP_AWARE)) {
                RETURN_SCORE_MINUS(10);
            }
            if ((AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_WATER_ABSORB, aiData)
             || AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_DRY_SKIN, aiData)
             || AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_EARTH_EATER, aiData))
             && !(AI_THINKING_STRUCT->aiFlags[battlerAtk] & AI_FLAG_HP_AWARE)) {
                RETURN_SCORE_MINUS(10);
            }
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_WATER_COMPACTION, aiData)) {
            ...
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_ANGER_POINT, aiData)
            ...
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_CONTRARY, aiData)
             && IsStatLoweringEffect(effect)) {
                RETURN_SCORE_PLUS(DECENT_EFFECT);
            }
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_MOTOR_DRIVE, aiData)
             && moveType == TYPE_ELECTRIC && BattlerStatCanRise(battlerAtkPartner, atkPartnerAbility, STAT_SPEED)) {
                RETURN_SCORE_PLUS(WEAK_EFFECT);
            }
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_LIGHTNING_ROD, aiData)
            ...
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_STORM_DRAIN, aiData)
             && (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5
             && moveType == TYPE_WATER
             && HasMoveWithCategory(battlerAtkPartner, DAMAGE_CATEGORY_SPECIAL)
             && BattlerStatCanRise(battlerAtkPartner, atkPartnerAbility, STAT_SPATK))) {
            ...
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_FLASH_FIRE, aiData)
             && moveType == TYPE_FIRE
             && HasMoveWithType(battlerAtkPartner, TYPE_FIRE)
             && !(gBattleResources->flags->flags[battlerAtkPartner] & RESOURCE_FLAG_FLASH_FIRE)) {
                RETURN_SCORE_PLUS(WEAK_EFFECT);
            }
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_SAP_SIPPER, aiData)
             && moveType == TYPE_GRASS
             && HasMoveWithCategory(battlerAtkPartner, DAMAGE_CATEGORY_PHYSICAL)
             && BattlerStatCanRise(battlerAtkPartner, atkPartnerAbility, STAT_ATK)) {
                RETURN_SCORE_PLUS(WEAK_EFFECT);
            }
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_JUSTIFIED, aiData)
             && moveType == TYPE_DARK
             && !IS_MOVE_STATUS(move)
             && HasMoveWithCategory(battlerAtkPartner, DAMAGE_CATEGORY_PHYSICAL)
             && BattlerStatCanRise(battlerAtkPartner, atkPartnerAbility, STAT_ATK)
            ...
            if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_RATTLED, aiData)
             && !IS_MOVE_STATUS(move)
             && (moveType == TYPE_DARK || moveType == TYPE_GHOST || moveType == TYPE_BUG)
        ...
        if (!partnerProtecting)
        {
            ...
            case EFFECT_SKILL_SWAP:
                ...
                {
                    if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_TRUANT, aiData))
                    {
                        RETURN_SCORE_PLUS(10);
                    }
                    else if (AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_COMPOUND_EYES, aiData)
            ...
            case EFFECT_ENTRAINMENT:
                ...
                break;
            case EFFECT_SOAK:
                if (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_WONDER_GUARD, aiData)
...
static u32 AI_CalcMoveEffectScore(u32 battlerAtk, u32 battlerDef, u32 move)
{
    ...
    if (AI_THINKING_STRUCT->aiFlags[battlerAtk] & AI_FLAG_SMART_SWITCHING && AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_NATURAL_CURE, AI_DATA))
    {
    ...
    case EFFECT_EVASION_DOWN_2:
        ...
        if (gBattleMons[battlerAtk].statStages[STAT_ACC] < DEFAULT_STAT_STAGE)
            ADJUST_SCORE(WEAK_EFFECT);
        if (gBattleMons[battlerDef].statStages[STAT_EVASION] < 7 || AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_NO_GUARD, aiData))
            ADJUST_SCORE(-2);
        break;
    case EFFECT_SPICY_EXTRACT:
    ...
    case EFFECT_HAZE:
        ...
        break;
    case EFFECT_ROAR:
        if ((gMovesInfo[move].soundMove && AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_SOUNDPROOF, aiData))
          || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_SUCTION_CUPS, aiData))
            break;
        else if (GetActiveGimmick(battlerDef) == GIMMICK_DYNAMAX)
            break;
        score += AI_TryToClearStats(battlerAtk, battlerDef, isDoubleBattle);
    ...
    case EFFECT_TRIPLE_KICK:
        if (AI_MoveMakesContact(battlerAtk, aiData->abilities[battlerAtk], aiData->holdEffects[battlerAtk], move)
          && !AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_MAGIC_GUARD, aiData)
    ...
    case EFFECT_REST:
        ...
        else if (ShouldRecover(battlerAtk, battlerDef, move, 100))
        {
            if (aiData->holdEffects[battlerAtk] == HOLD_EFFECT_CURE_SLP
              ...
              || AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_SHED_SKIN, aiData)
              || AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_EARLY_BIRD, aiData)
              || (AI_GetWeather(aiData) & B_WEATHER_RAIN && gWishFutureKnock.weatherDuration != 1 && AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_HYDRATION, aiData) && aiData->holdEffects[battlerAtk] != HOLD_EFFECT_UTILITY_UMBRELLA))
                ADJUST_SCORE(GOOD_EFFECT);
        }
        break;
    case EFFECT_OHKO:
    ...
    case EFFECT_FOCUS_ENERGY:
    case EFFECT_LASER_FOCUS:
        if (AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_SUPER_LUCK, aiData)
          || AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_SNIPER, aiData)
    ...
    case EFFECT_LEECH_SEED:
        if (IS_BATTLER_OF_TYPE(battlerDef, TYPE_GRASS)
          ...
          || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_LIQUID_OOZE, aiData)
          || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_MAGIC_GUARD, aiData))
            break;
        ADJUST_SCORE(GOOD_EFFECT);
    ...
    case EFFECT_PROTECT:
        ...
        case MOVE_KINGS_SHIELD:
            if (AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_STANCE_CHANGE, aiData) //Special logic for Aegislash
    ...
    case EFFECT_TOXIC_SPIKES:
        ...
        break;
    case EFFECT_FORESIGHT:
        if (AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_SCRAPPY, aiData) || AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_MINDS_EYE, aiData))
            break;
    ...
    case EFFECT_FELL_STINGER:
        if (gBattleMons[battlerAtk].statStages[STAT_ATK] < MAX_STAT_STAGE
        && !AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_CONTRARY, aiData)
    ...
    case EFFECT_BESTOW:
        ...
        case HOLD_EFFECT_UTILITY_UMBRELLA:
            if (!AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_SOLAR_POWER, aiData) && !AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_DRY_SKIN, aiData))
            {
                if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_SWIFT_SWIM, aiData)
                 && AI_GetWeather(aiData) & B_WEATHER_RAIN) {
                    ADJUST_SCORE(DECENT_EFFECT);
                }
                if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_CHLOROPHYLL, aiData)
                 && AI_GetWeather(aiData) & B_WEATHER_SUN) {
                    ADJUST_SCORE(DECENT_EFFECT);
                }
                if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_FLOWER_GIFT, aiData)
                 && AI_GetWeather(aiData) & B_WEATHER_SUN) {
                    ADJUST_SCORE(DECENT_EFFECT);
                }
            }
    ...
    case EFFECT_RECYCLE:
        ...
        if (IsRecycleEncouragedItem(GetUsedHeldItem(battlerAtk)))
            ADJUST_SCORE(WEAK_EFFECT);
        if (AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_RIPEN, aiData))
        {
    ...
    case EFFECT_TICKLE:
        if (gBattleMons[battlerDef].statStages[STAT_DEF] > 4 && HasMoveWithCategory(battlerAtk, DAMAGE_CATEGORY_PHYSICAL)
         && !AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_CONTRARY, aiData) && ShouldLowerDefense(battlerAtk, battlerDef, aiData->abilities[battlerDef]))
            ADJUST_SCORE(DECENT_EFFECT);
    ...
    case EFFECT_SOAK:
        ...
        break;
    case EFFECT_THIRD_TYPE:
        if (AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_WONDER_GUARD, aiData))
            ADJUST_SCORE(DECENT_EFFECT); // Give target more weaknesses
        break;
    case EFFECT_ELECTRIFY:
        if (predictedMove != MOVE_NONE
         && (AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_VOLT_ABSORB, aiData)
          || AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_MOTOR_DRIVE, aiData)
          || (B_REDIRECT_ABILITY_IMMUNITY >= GEN_5 && AI_BattlerHasAbilityOrInnate(battlerAtk, ABILITY_LIGHTNING_ROD, aiData))))
        {
            ADJUST_SCORE(DECENT_EFFECT);
        }
        break;
        ...
        else // consider move effects that hinder the target
        {
            switch (gMovesInfo[move].additionalEffects[i].moveEffect)
            {
                ...
                case MOVE_EFFECT_BUG_BITE:   // And pluck
                    if (gBattleMons[battlerDef].status2 & STATUS2_SUBSTITUTE || AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_STICKY_HOLD, aiData))
                        break;
                ...
                case MOVE_EFFECT_STEAL_ITEM:
                    {
                        ...
                        && !AI_BattlerHasAbilityOrInnate(battlerDef, ABILITY_STICKY_HOLD, aiData))
                        {
                            switch (aiData->holdEffects[battlerDef])
                            {
...
static s32 AI_HPAware(u32 battlerAtk, u32 battlerDef, u32 move, s32 score)
{
    ...
    if (IS_TARGETING_PARTNER(battlerAtk, battlerDef))
    {
        if ((effect == EFFECT_HEAL_PULSE || effect == EFFECT_HIT_ENEMY_HEAL_ALLY)
         || (moveType == TYPE_ELECTRIC && AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_VOLT_ABSORB, AI_DATA))
         || (moveType == TYPE_GROUND && AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_EARTH_EATER, AI_DATA))
         || (moveType == TYPE_WATER && (AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_DRY_SKIN, AI_DATA) || AI_BattlerHasAbilityOrInnate(BATTLE_PARTNER(battlerAtk), ABILITY_WATER_ABSORB, AI_DATA))))
        {
            if (gStatuses3[battlerDef] & STATUS3_HEAL_BLOCK)
                return 0;
